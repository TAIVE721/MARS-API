...
CREATE TABLE ElementsToMars (
    id BINARY(16) PRIMARY KEY DEFAULT(UUID_TO_BIN(UUID())), Category INT NOT NULL REFERENCES Categorys (id), name VARCHAR(255) NOT NULL, weight INTEGER NOT NULL, description VARCHAR(255), FOREIGN KEY (Category) REFERENCES Categorys (id)
);
